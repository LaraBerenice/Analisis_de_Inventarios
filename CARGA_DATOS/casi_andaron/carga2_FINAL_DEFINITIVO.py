...
# Crear tablas con claves autoincrementales
create_table_sqls = [
    '''
    ...
    CREATE TABLE dbo.Fact_Ventas (
        VentasID INT PRIMARY KEY IDENTITY(1,1),
    ...
    CREATE TABLE dbo.Fact_Compras (
        ComprasID INT PRIMARY KEY IDENTITY(1,1),
        InventoryId VARCHAR(255),
        Store VARCHAR(255),
        ReceivingDate DATE,
        Quantity FLOAT,
...
def insertar_datos(df, tabla):
    ...
    if 'InvInicioID' in df_validado.columns:
        df_validado = df_validado.drop(columns=['InvInicioID'])
    if 'InvFinYearID' in df_validado.columns:
        df_validado = df_validado.drop(columns=['InvFinYearID'])

    if not df_validado.empty:
        try:
            with Session() as session:
                df_validado.to_sql(tabla, con=session.bind, if_exists='append', index=False)
                print(f"Datos insertados en {tabla} con éxito.")
        except SQLAlchemyError as e:
            print(f"Error al insertar datos en {tabla}: {str(e)}")
    else:
        print(f"No hay datos válidos para insertar en {tabla}.")

# Insertar datos en tablas de dimensión primero
for tabla, df in zip(['DimVendor', 'Dim_Producto', 'Dim_OC'], [df_vendor, df_producto, df_oc]):
    insertar_datos(df, tabla)

# Insertar datos en tablas de hechos
for tabla, df in zip(['Fact_Ventas', 'Fact_Compras', 'Fact_InvInicio', 'Fact_InvFinYear'], [df_ventas, df_compras, df_inv_inicio, df_inv_fin_year]):
    if df is not None:
